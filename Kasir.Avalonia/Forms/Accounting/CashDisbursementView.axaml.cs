...
public partial class CashDisbursementView : UserControl
{
    ...
    private async void SaveTransaction()
    {
        if (_lines.Count == 0)
        {
            await MsgBox.Show(NavigationService.Owner, "Belum ada baris transaksi.");
            return;
        }

        string docDate = TxtDate.Text?.Trim() ?? DateTime.Now.ToString("yyyy-MM-dd");
        string remark  = TxtRemark.Text?.Trim() ?? "";
        string prefix  = _isBankMode ? "BKL" : "KKL";
        string docType = _isBankMode ? "BANK_OUT" : "CASH_OUT";
        ...
        try
        {
            ...
            await MsgBox.Show(NavigationService.Owner, $"Transaksi {journalNo} berhasil disimpan.");
            ClearForm();
        }
        catch (Exception ex)
        {
            await MsgBox.Show(NavigationService.Owner, "Gagal menyimpan: " + ex.Message);
        }
    }
