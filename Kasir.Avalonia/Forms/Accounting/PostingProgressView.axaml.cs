...
public partial class PostingProgressView : UserControl
{
    ...
    private async void RunClosePeriod()
    {
        try
        {
            bool c = await MsgBox.Confirm(NavigationService.Owner, $"Tutup periode {_periodCode}? Tidak bisa dibatalkan.");
            if (!c) return;
