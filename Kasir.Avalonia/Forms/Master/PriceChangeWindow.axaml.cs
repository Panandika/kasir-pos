...
public partial class PriceChangeWindow : Window
{
    ...
    private async void EditSelectedPrice()
    {
        ...
        if (row == null) { await MsgBox.Show(this, "Pilih produk dulu."); return; }
    ...
    private async void SaveChanges()
    {
        ...
        if (changes.Count == 0) { await MsgBox.Show(this, "Tidak ada perubahan harga."); return; }

        bool confirmed = await MsgBox.Confirm(this, $"Simpan {changes.Count} perubahan harga?");
        if (!confirmed) return;

        int count = _priceService.ApplyBatchPriceChange(changes, 1);
        await MsgBox.Show(this, $"{count} harga berhasil diubah.");
        LoadProducts();
    }
