...
namespace Kasir.Forms.Purchasing
{
    ...
    {
        private TextBox txtVendor, txtDate, txtDueDate, txtTerms;
        ...
        public PurchaseInvoiceForm()
        {
            ...
            InitializeLayout();
            SetAction("Nota Pembelian — F2: Vendor, Ins: Add Item, F10: Save (creates AP), Esc: Close");
        ...
        private void AddItem()
        {
            ...
            using (var dlg = new InputDialog("Item", new[] { "Qty", "Harga" }, new[] { "1", (product.BuyingPrice / 100.0).ToString("F0") }))
            {
                if (dlg.ShowDialog(this) == DialogResult.OK)
                {
                    int qty; double price;
                    int.TryParse(dlg.Values[0], out qty); double.TryParse(dlg.Values[1], out price);
        ...
        private void Save()
        {
            ...
            int terms; int.TryParse(txtTerms.Text, out terms);
            var invoice = new Purchase { SubCode = _vendorCode, DocDate = txtDate.Text.Trim(), DueDate = txtDueDate.Text.Trim(), Terms = terms };
            string jnl = _service.CreatePurchaseInvoice(invoice, _items, 1);
            MessageBox.Show("Invoice saved: " + jnl + "\nAP entry created.");
